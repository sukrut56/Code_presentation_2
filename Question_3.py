...
def three_one(dataset):
	x = np.array([dataset])	#converting input to array using np.array() method 
	s = (x[0, ::25])	#selecting values from 0 to length of total array with a seperation of 25 indexes
...
def three_one1(array):
	length = len(array)		#checking the length of array 
	s = (array[0:length:25])	#selecting values from 0 to length of array with seperation of 25 indexes 
